...
export default function Counters() {
  const counters = [
    {
      ...
      logo: (
        <div className="relative h-[7.5rem] w-[7.5rem] flex items-center justify-center">
          <img
            src="/Assets/icons/round_counters.svg"
            alt="round icon"
            className="absolute inset-0 w-full h-full"
          />
          <LottieAnimation autoplay loop animationData={people} width="4.5rem"  height="4.5rem"/>
        </div>
      ),
      suffix: "k+",
      duration: 2,
      ...
      logo: (
        <div className="relative h-[7.5rem] w-[7.5rem] flex items-center justify-center">
          <img
            ...
            className="absolute inset-0 w-full h-full"
          />
          <LottieAnimation autoplay loop animationData={live}  width="6rem"  height="6rem"/>
        </div>
      ),
      suffix: " Million+",
      duration: 2.5,
      ...
      logo: (
        <div className="relative h-[7.5rem] w-[7.5rem] flex items-center justify-center">
          <img
            ...
            className="absolute inset-0 w-full h-full"
          />
          <LottieAnimation autoplay loop animationData={youtube}  width="4.5rem"  height="4.5rem"/>
        </div>
      ),
      suffix: " Million+",
      duration: 2,
  ...
  return (
    <Center className="bg-[#F6FDFE]">
      <div
        ref={ref}
        className="grid gap-4 p-4 common-space-x py-8 w-full place-items-center justify-center grid-cols-1 md:grid-cols-2 lg:grid-cols-[15rem,15rem,15rem,15rem]"
      >
        {counters.map((data) => (
          <div
            key={data.subtitle}
            className="border border-black p-6 rounded-md bg-white flex flex-col items-center w-[20rem]  md:w-full lg:w-[15rem] px-0"
          >
            <div className="relative">{data.logo}</div>
